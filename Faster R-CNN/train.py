...
class Train:
    ...
    def train(self):
        ...
        while iter < cfg.FLAGS.max_iters + 1:
            ...
            timer.toc()
            rpn_loss_cls_list.append(rpn_loss_cls)
            rpn_loss_box_list.append(rpn_loss_box)
            ...
            if iter % cfg.FLAGS.snapshot_iterations == 0:
                self.snapshot(sess, iter )
                dataframe=pd.DataFrame({'rpn_loss_cls': rpn_loss_cls_list,'rpn_loss_box':rpn_loss_box_list, 'loss_cls':loss_cls_list,'loss_box':loss_box_list,'total_loss':total_loss_list})
                dataframe.to_csv("loss%d.csv" %iter)
